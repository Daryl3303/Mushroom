...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(sensorDataRef, (snapshot) => {
      ...
      if (data) {
        // Parse NPK values from strings (e.g., "48.0(494.0)" -> 48.0)
        const nitrogen = parseFloat(data.npk.nitrogen.split("(")[0]);
        const phosphorus = parseFloat(data.npk.phosphorus.split("(")[0]);
        const potassium = parseFloat(data.npk.potassium.split("(")[0]);
        const ph = parseFloat(data.ph.split("(")[0]);
